...
describe("API de Crianças", () => {
    ...
    test("POST /api/criancas - Criar criança", async () => {
        ...
        expect(response.body.dataNascimento).toContain(
            novaCrianca.dataNascimento,
        );
    });
    ...
    test("GET /api/criancas - Listar crianças", async () => {
        await dbRun(
            'INSERT INTO criancas (nome, "dataNascimento", responsavel, telefone) VALUES ($1, $2, $3, $4) RETURNING id',
            ["João", "2018-02-12", "Maria", "11999999999"],
        );
        await dbRun(
            'INSERT INTO criancas (nome, "dataNascimento", responsavel, telefone) VALUES ($1, $2, $3, $4) RETURNING id',
            ["Ana", "2016-05-20", "Pedro", "11888888888"],
        );
    ...
    test("GET /api/criancas/buscar?nome=Silva - Buscar crianças por nome", async () => {
        await dbRun(
            'INSERT INTO criancas (nome, "dataNascimento", responsavel, telefone) VALUES ($1, $2, $3, $4) RETURNING id',
            ["João Silva", "2018-02-12", "Maria", "11999999999"],
        );
        await dbRun(
            'INSERT INTO criancas (nome, "dataNascimento", responsavel, telefone) VALUES ($1, $2, $3, $4) RETURNING id',
            ["Ana Santos", "2016-05-20", "Pedro", "11888888888"],
        );

        const response = await request(app)
            .get("/api/criancas/buscar?nome=Silva")
            .expect(200);

        expect(Array.isArray(response.body)).toBe(true);
        expect(response.body.length).toBe(1);
        expect(response.body[0].nome).toContain("Silva");
    });
    ...
    test("PUT /api/criancas/:id - Atualizar criança", async () => {
        ...
        expect(response.body.dataNascimento).toContain("2017-02-12");
    });
