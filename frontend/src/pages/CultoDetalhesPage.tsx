...
const CultoDetalhesPage = () => {
  ...
  const criancasFiltradas = criancas
    .filter((crianca) => {
      ...
      const nomeNormalizado = normalizarTexto(crianca.nome);

      const nomeMatch = nomeNormalizado.includes(termoNormalizado);

      const codigoMatch = crianca.codigo
        ? String(crianca.codigo).includes(termoNormalizado)
        : false;
  ...
  const criancasPresentes = criancasFiltradas.filter(
    (crianca) => !crianca.checkedOut,
  ).length;
  ...
  const handleRemoverCrianca = (criancaId: number) => {
    ...
    if (
      window.confirm("Tem certeza que deseja remover esta criança do culto?")
    ) {
      cultosAPI
  ...
  const handleGerarEtiqueta = (crianca: Crianca, tipo: "filho" | "pai") => {
    ...
    const nomeFontSize = tipo === "filho" ? "26px" : "32px";
    ...
    const icones = [
      !crianca.autorizaUsoImagem &&
        `<img alt="" src="${window.location.origin}/icones/${autorizaImagem}" title="Autoriza uso de imagem" style="width:48px;height:48px;object-fit:contain;" />`,
    ...
    const etiquetaHTML = `
        ...
        <head>
            ...
            <style>
                @page { size: 80mm 40mm portrait; margin: 0; }
                * { margin: 0; padding: 0; box-sizing: border-box; }
                html, body { width: 80mm; height: 30mm; }
                ...
                .info { font-size: 24px; color: #555; text-align: center; }
                ...
                >
                  <div className="flex-1 flex gap-6 flex-col ">
                    <div className="flex gap-2">
                      (
                      <button
                        onClick={() => handleMarcarCheckout(crianca)}
                      ...
                      >
                        {crianca.checkedOut ? "✓ Checkout" : "Checkout"}
                      </button>
                      )
                      <button
                        onClick={() => handleGerarEtiqueta(crianca, "filho")}
                        className="px-3 py-1 rounded text-sm font-medium bg-green-500 hover:bg-green-600 text-white transition-colors"
                      >
                        Etiqueta Filho
                      </button>
                      <button
                        onClick={() => handleGerarEtiqueta(crianca, "pai")}
                      ...
                      <button
                        onClick={() =>
                          crianca.id && handleRemoverCrianca(crianca.id)
                        }
                        className={
                          crianca.checkedOut
                    ...
                    <div>
                      ...
                      >
                        {crianca.codigo
                          ? `${String(crianca.codigo).padStart(
                              2,
                              "0",
                      ...
                      <div className="text-sm text-gray-600 mt-1 ml-0">
                        <div>
                          ...
                          {crianca.restricaoAlimentar
                            ? crianca.descricaoRestricaoAlimentar || "Sim"
                            : "Não"}
                        </div>
                        <div>
                          <strong>Necessidade Especial:</strong>{" "}
                          {crianca.necessidadeEspecial
                            ? crianca.descricaoNecessidadeEspecial || "Sim"
                            : "Não"}
                        </div>
                        <div>
                          <strong>Autoriza uso de imagem:</strong>{" "}
                          {crianca.autorizaUsoImagem ? "Sim" : "Não"}
                        </div>
                        <div>
                          <strong>Autoriza troca de fralda:</strong>{" "}
                          {crianca.autorizaTrocaFralda ? "Sim" : "Não"}
                        </div>
                      </div>
                    </div>
                  </div>
